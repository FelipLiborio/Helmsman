...
import {
  ...
  fetchContainers, fetchDecisions, fetchAlerts, fetchHttpStats, fetchHostInfo, fetchAgStatus,
  type Metrics, type Status, type HistoryPoint,
  type ContainerStat, type Decision, type Alert, type HttpStats, type HostInfo, type AgStatus,
} from "../services/api";
...
export default function AGDashboard() {
  ...
  const [agStatus,   setAgStatus]   = useState<AgStatus | null>(null);

  const refresh = useCallback(async () => {
    const [m, s, h, c, d, a, hs, hi, ag] = await Promise.all([
      fetchMetrics(),
      fetchStatus(),
      fetchHistory(120),
      fetchContainers(),
    ...
    if (ag && ag.mode === "ag") setAgStatus(ag);
  }, [decisionLimit]);
  ...
  return (
    <div style={{ background: "#f8f8f6", minHeight: "100vh", padding: 16, fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 15, fontWeight: 500 }}>
            <span style={{ color: "#EF9F27" }}>⬡</span> Helmsman
            {serviceName && <span style={{ color: "#aaa", fontWeight: 400 }}> · {serviceName}</span>}
          </span>
          <span style={{
            fontSize: 10, padding: "2px 8px", borderRadius: 99,
            background: "#FFF4E0", color: "#BA7517", border: "0.5px solid #EF9F27", fontWeight: 500,
          }}>AG</span>
      ...
      {/* último ciclo AG */}
      <SectionLabel>último ciclo AG</SectionLabel>
      {agStatus ? (
        <div style={{ marginBottom: 12 }}>
          <Card>
            <CardTitle>cromossomo ótimo</CardTitle>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div>
                <div style={{ fontSize: 10, color: "#aaa", marginBottom: 2 }}>réplicas sugeridas</div>
                <div style={{ fontSize: 28, fontWeight: 500, color: "#1a1a1a", lineHeight: 1 }}>{agStatus.replicas_alvo}</div>
              </div>
              <div>
                <div style={{ fontSize: 10, color: "#aaa", marginBottom: 2 }}>sla threshold evoluído</div>
                <div style={{ fontSize: 22, fontWeight: 500, color: "#EF9F27", lineHeight: 1 }}>{agStatus.sla_threshold}%</div>
              </div>
              <div style={{ borderTop: "0.5px solid #f0f0ee", paddingTop: 8, display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontSize: 10, color: "#aaa" }}>fitness final</span>
                <span style={{ fontSize: 10, color: "#1a1a1a", fontFamily: "monospace" }}>{agStatus.melhor_fitness.toFixed(3)}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontSize: 10, color: "#aaa" }}>gerações</span>
                <span style={{ fontSize: 10, color: "#1a1a1a" }}>{agStatus.geracoes}</span>
              </div>
            </div>
          </Card>
        </div>
        ...
        <Card style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 12, color: "#aaa", padding: "8px 0" }}>aguardando primeiro ciclo AG...</div>
        </Card>
      )}
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={140}>
            <AreaChart data={chartData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ fontSize: 11, borderRadius: 6, border: "0.5px solid #e0e0de" }} />
              <Area type="stepAfter" dataKey="replicas" stroke="#EF9F27" fill="#FFF4E0" strokeWidth={2} dot={false} name="réplicas" />
            </AreaChart>
          </ResponsiveContainer>
        </Card>
      </div>
      ...
      {/* decisões AG */}
      <SectionLabel>decisões AG recentes</SectionLabel>
      <Card style={{ marginBottom: 12 }}>
